...
int binmid   = 8;  // raw signal directly from histP up to this bin    .......   TPC??
...
const char* list = "AProtons"; //
const char* part = "aprot"; //
...
void MakeRawSignalaProt(){
    ...
    // correct raw spectrum for PID purity ..... TOF PID pt range 0.7 to 2.5 GeV/c    Q:: what does it make exactly??
    for (int i=8;i<=24;i++){
        ...
        std::cout<<"***********Correction for PID Purity *************"<<std::endl;
        std::cout<<"hist_tofcut_p Bin Content = "<<hist_tofcut_p->GetBinContent(i)<<"purity bin content = "<<purity->GetBinContent(i)<<"hist_tofcut_p bin error = "<<hist_tofcut_p->GetBinError(i)<<std::endl;
    }
...
void FillSpectrumLowP(TH3F* hist, TH1F* rawSpectrum){
    ...
    // cut on DCAxy
    hDCAxy_p->GetYaxis()->SetRangeUser(DCAlow,DCAhigh);   // z-axis became y-axis after the projection..
...
void FitTOFm2Background(TH3F* hist, TH1F* rawSpectrum, TH1F* purity){
    ...
    // cut on DCA
    hist->GetZaxis()->SetRangeUser(DCAlow,DCAhigh);  // hist is the 3-D histogram
    // 2D hist
    ...
    TF1 *fitFcnBckgrndExc = new TF1("fitFcnBckgrndExc",TOFbackgroundExcludePeak,xmin,xmax,8);  // 8 is step/bin width?
    // start values for bin 15
    Double_t StartValuesBackground[8] = {1166, -424, 79148, -7, 343566, 0.24, 0.02, 1.0};   // bin 15 in TOF PID region
    ...
    // yield and yield error    ....   why do we want to assign these values
    Double_t yield = 1.0;
    ...
    Int_t nBins = 200;    // which momentum bins??  is it up to 2 GeV/c??
    TH1F* fHistHelp = new TH1F("fHistHelp","fHistHelp",nBins,0,xmax);  // xmax = 2.0

    // canvas
    TCanvas* c1 = new TCanvas("c1","c1");
    c1->SetLogy();

    // my trial to see fHistHelp
    fHistHelp->Draw();  // should it be here or in the main??

    // ---------------------------------------------------------------

    // fit in momentum bins starting from bin 1.4-1.5 to 2.5
    for (Int_t ibin = 15; ibin <= binmid2-1; ibin++){    // binmid2  = 26
        ...
        hTOFm2->GetFunction("fitFcnBckgrndExc")->Delete();  // why??
        ...
        fTOFbackground.DrawCopy("same");   // what is DrawCopy function??
    ...
    for (Int_t ibin = 14; ibin >= binmid; ibin--){
        ...
        for (Int_t i = 0; i <= 7; i++){
            fTOFbackground.SetParameter(i,fitFcnBckgrndExc->GetParameter(i));
            std::cout<<********fTOFbackground**********<<std::endl;
            std::cout<<fitFcnBckgrndExc->GetParameter(i)<<std::endl;
        }
