...
void MakeRawSignalaProt(){
  ...
  //TH1F* hist_tofcut_p = (TH1F*)file->Get(outp)->FindObject(list)->FindObject("fHistP")->Clone("hist_tofcut_p");
   ...
   //TH1F* hist_tofcut_p = (TH1F*)file->Get(outp_tof)->FindObject(list)->FindObject("fHistP")->Clone("hist_tofcut_p");
...
void FitTOFm2Background(TH3F* hist, TH1F* rawSpectrum, TH1F* purity){
    ...
    double d = TOFbackgroundExcludePeak;
    cout<<d<<endl;
    ...
    for (Int_t ibin = 15; ibin <= binmid2-1; ibin++){
        ...
        hTOFm2->Fit("fitFcnBckgrndExc","R","",xmin,xmax);
        ...
        hTOFm2->GetFunction("fitFcnBckgrndExc");
